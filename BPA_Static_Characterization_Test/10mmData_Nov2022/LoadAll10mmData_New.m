...
li = [8.3,7.975,7.65,7.325];
    ...
    for a = 1:length(kink_p)
        for b=1:length(test)
            ...
            BPA10mm10cm{b,a}(:,7) = ones(length(BPA10mm10cm{b,a}),1)*lo; %resting length(lo) 
            ...
            BPA10mm10cm{b,a}(:,14) = ones(length(BPA10mm10cm{b,a}),1)*test_num(b); %test number
            BPA10mm10cm{b,a}=imresize(BPA10mm10cm{b,a},[num 14],'nearest') %resize it all to remove excess data (already checked for accuracy
        end
    end
