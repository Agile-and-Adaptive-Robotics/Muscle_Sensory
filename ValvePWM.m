...
function [Data, Stats] = KneeTest(protocol_id, varargin) 
     p = inputParser;
     addRequired(p,'protocol_id');
     addOptional(p,'total',10000);
     addOptional(p,'pw',0);
     addOptional(p,'period',1000);
     parse(p, protocol_id, varargin{:});
    ...
    if protocol_id == 2   %the rest is for the knee torque test
        ...
        vars = strjoin({num2str(total), num2str(pw), num2str(period)},',');
