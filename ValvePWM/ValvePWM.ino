/* sketch adapted by Lindie Burgess for use by the Agile and Adaptive Robotics Lab at Portland State
  ...
  void loop() {
    ...
    {     
      choose_branch = Serial.read();        //If there's information sent over serial from Matlab, read the serail data into variable "choose branch"
      
      if (choose_branch == '2') 
      {           //If choose_branch =='2', then proceed to the following while loop. 
        Serial.println("running");          //Tell matlab that the arduino recieved the protocol id = running. 
        String reading = "2";               //initiate variable to store the serial data when it is being read
 
          while (true) 
          {
             reading = Serial.readStringUntil(',');       //read from serial until next , sign
                if (reading.length() > 1 and reading != "2\n") 
                {   //if the reading is anything but 2 then break and start collecting data (we need to ignore 2 because matlab sends many instances of the protocol id, so we want to make sure that it checks for the next unique value, which would be how many data points to collect)
                  break;        
                }
          }
  
          total = reading.toInt();                                   //total = convertion of 'reading' string to an integer
          pw_in = Serial.readStringUntil(',').toInt();               //pw_in = 
          ...
          double timer = 0;
                ...
                { //data collection starts, collect 1/2 of total data points
                  timer = millis() - start;
          
                  if (fmod(timer,period_in.toInt())<= pw_in.toInt())      //fmod = floating point modulus.
                  {       
